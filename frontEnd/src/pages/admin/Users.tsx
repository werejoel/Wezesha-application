...
export default function AdminUsers() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'enumerator' });
  const [formError, setFormError] = useState<string | null>(null);
  const [editOpen, setEditOpen] = useState(false);
  const [editingUser, setEditingUser] = useState<any | null>(null);
  const [editForm, setEditForm] = useState({ name: '', email: '', password: '', role: 'enumerator' });
  ...
  useEffect(() => {
    ...
    if (!form.password || form.password.length < 8 || form.password.length > 128) errs.password = 'Password must be 8-128 characters';
  ...
  const handleCreate = async () => {
    ...
    try {
      const created = await createUser(form.name, form.email, form.password, form.role);
      ...
      toast({ title: 'Create failed', description: String(err?.message || 'Failed to create user') });
    }
  };
  ...
  const handleToggleRole = async (u: any) => {
    ...
    try {
      ...
      toast({ title: 'User updated', description: `${updated.email} role set to ${updated.role}` });
    } catch (err: any) {
      setError(err?.message || 'Failed to update user');
      toast({ title: 'Update failed', description: String(err?.message || 'Failed to update user') });
    }
  };

  const openEdit = (u: any) => {
    setEditingUser(u);
    setEditForm({ name: u.name || u.full_name || '', email: u.email || '', password: '', role: u.role || 'enumerator' });
    setEditError(null);
    setEditFieldErrors({});
    setEditOpen(true);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    if (editForm.password && (editForm.password.length < 8 || editForm.password.length > 128)) errs.password = 'Password must be 8-128 characters';
    ...
    <div className="space-y-6">
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-description">Create, view and manage application users (admin only)</p>
        </div>
        <div>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <div className="grid gap-3">
                ...
                <div>
                  ...
                  {createFieldErrors.name && <p className="text-xs text-destructive mt-1">{createFieldErrors.name}</p>}
                </div>
                <div>
                  <Label>Email</Label>
                ...
                <div>
                  <Label>Role</Label>
                  <select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} className="w-full rounded-lg border bg-card px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring">
                ...
                <div className="flex justify-end gap-2">
                  ...
                  <Button onClick={handleCreate} disabled={Object.keys(createFieldErrors).length > 0}>Create</Button>
                </div>
              </div>
            </DialogContent>
          </Dialog>
          <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
            <DialogContent className="sm:max-w-sm">
              ...
              <div className="grid gap-3">
                <p>Are you sure you want to delete <strong>{deleteTarget?.email || deleteTarget?.id}</strong>? This action cannot be undone.</p>
                <div className="flex justify-end gap-2">
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <div className="grid gap-3">
                ...
                <div>
                  ...
                  {editFieldErrors.name && <p className="text-xs text-destructive mt-1">{editFieldErrors.name}</p>}
                </div>
                <div>
                  <Label>Email</Label>
                  <Input value={editForm.email} onChange={e => setEditForm({ ...editForm, email: e.target.value })} type="email" />
                  {editFieldErrors.email && <p className="text-xs text-destructive mt-1">{editFieldErrors.email}</p>}
                </div>
                <div>
                  <Label>Password (leave blank to keep)</Label>
                  <Input value={editForm.password} onChange={e => setEditForm({ ...editForm, password: e.target.value })} type="password" />
                  {editFieldErrors.password && <p className="text-xs text-destructive mt-1">{editFieldErrors.password}</p>}
                </div>
                <div>
                  <Label>Role</Label>
                  <select value={editForm.role} onChange={e => setEditForm({ ...editForm, role: e.target.value })} className="w-full rounded-lg border bg-card px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring">
                    <option value="admin">admin</option>
                    <option value="program_manager">program_manager</option>
                    <option value="ybf">ybf</option>
                    <option value="instructor">instructor</option>
      ...
      {error && <div className="rounded-lg border border-destructive/10 bg-destructive/5 p-3 text-sm text-destructive">{error}</div>}
      ...
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setPage(1); }} className="rounded-lg border bg-card px-2 py-1 text-sm">
        ...
        <CardContent className="p-0 overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-8 text-sm text-muted-foreground">Loading users...</TableCell>
                </TableRow>
                ) : users.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-8 text-sm text-muted-foreground">No users found.</TableCell>
                </TableRow>
                ) : (
                  users.map(u => (
                  <TableRow key={u.id}>
                    <TableCell className="font-medium">{u.name || u.full_name || u.email}</TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="sm" onClick={() => handleToggleRole(u)}>{u.role === 'admin' ? 'Demote' : 'Promote'}</Button>
                        <Button variant="destructive" size="sm" onClick={() => handleDelete(String(u.id))}>Delete</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))
