...
export default function AdminDashboard() {
  ...
  const downloadExportFile = async (resource: string, filename: string) => {
    try {
      ...
      toast({
        title: "Export failed",
        description: error?.message || "Unable to download data.",
      });
    }
  };
  ...
  const recentActivity = useMemo(() => {
    const baseActivity = [
      ...
      ...((youthData || []).slice(0, 2).map((y: any) => ({
        ...
        time: y.enrolment_date
          ? new Date(y.enrolment_date).toLocaleDateString()
          : "Unknown date",
      })) as any[]),
    ];
    return baseActivity;
  ...
  const pendingSyncs =
    typeof dashboardStats.pendingSyncs === "number"
      ? dashboardStats.pendingSyncs
      : 0;
  const atRiskCount =
    typeof atRiskListData?.total === "number"
      ? atRiskListData.total
      : typeof dashboardStats.atRiskCount === "number"
        ? dashboardStats.atRiskCount
        : filteredAtRiskYouth.length;
  const systemUsers =
    typeof usersTotal === "number"
      ? usersTotal
      : typeof dashboardStats.totalUsers === "number"
        ? dashboardStats.totalUsers
        : users.length;
  const systemHealthScore = Math.max(
    45,
    100 - Math.min(60, pendingSyncs * 10 + atRiskCount * 4),
  );

  const genderData = [
    {
      name: "Female",
      value: youth.filter((y) => y.gender === "Female").length,
    },
  ...
  const userRoles = [
    {
      name: "YBF",
      value: users.filter((u: any) => u.role?.toLowerCase() === "ybf").length,
    },
    {
      name: "Instructor",
  ...
  const enrollmentByMonth = Array.from(enrollMap.values())
    .sort((a, b) => {
  ...
  const activityMap = new Map<
    string,
    { month: string; active: number; inactive: number }
  >();
  ...
  youth.forEach((y: any) => {
    ...
    if (!month) return;
    const item = activityMap.get(month.key) || {
      month: month.month,
      active: 0,
      inactive: 0,
    };
  ...
  const cumulativeUsage = Array.from(activityMap.values())
    .sort((a, b) => {
      const aDate = new Date(`${a.month} 1, 2000`);
      const bDate = new Date(`${b.month} 1, 2000`);
      return aDate.getMonth() - bDate.getMonth();
  ...
  const radarData = [
    ...
    {
      subject: "User Adoption",
      A: Math.min(100, (dashboardStats.totalUsers || users.length) * 10),
    },
    {
      subject: "Youth Engagement",
      A: Math.min(
        100,
        youth.length
          ? Math.max(
              20,
              100 - Math.round((atRiskCount / (youth.length || 1)) * 100),
            )
          : 0,
      ),
    },
  ...
  const lowAttendanceCount =
    typeof lowAttendanceData?.total === "number" ? lowAttendanceData.total : 0;
  ...
  const outcomeMetrics = [
    {
      label: "At-risk youth",
      value: youth.length ? Math.round((atRiskCount / youth.length) * 100) : 0,
      suffix: "%",
    },
    {
      label: "Low attendance",
      value: youth.length
        ? Math.round((lowAttendanceCount / youth.length) * 100)
        : 0,
      suffix: "%",
    },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 p-4 bg-muted/30 rounded-xl border border-border">
        ...
        {[
          ...
          { label: "Active Users", value: systemUsers, trend: "+12", up: true },
          {
            label: "Pending Syncs",
            value: pendingSyncs,
            trend: pendingSyncs > 0 ? `+${pendingSyncs}` : "0",
            up: pendingSyncs === 0 ? true : false,
          },
          {
            label: "Open Alerts",
            value: atRiskCount,
            trend: atRiskCount > 0 ? `+${atRiskCount}` : "0",
            up: atRiskCount === 0 ? true : false,
          },
        ].map((item) => (
          <div
            key={item.label}
            className="flex flex-col justify-center px-3 border-t sm:border-t-0 sm:border-l border-border pt-3 sm:pt-0"
          >
            ...
            <div className="space-y-2.5">
              {(() => {
                ...
                const lowAttendanceTotal =
                  typeof lowAttendanceData?.total === "number"
                    ? lowAttendanceData.total
                    : 0;
                if (
                  dashboardStats.avgAttendance !== null &&
                  dashboardStats.avgAttendance < 75
                ) {
                  derived.push({
                    title: "Low Attendance Alert",
                ...
                return combined.map((alert, i) => (
                  <div
                    key={`${alert.title}-${i}`}
                    className={cn(
                      "flex items-start justify-between p-2.5 rounded-lg border gap-3",
                      alert.bg,
                      alert.border,
                    )}
                  >
                    <div className="flex-1 min-w-0">
                      <p
                        className={cn("text-sm font-medium", alert.titleColor)}
                      >
                        {alert.title}
                      </p>
                      <p className={cn("text-xs mt-0.5", alert.descColor)}>
                        {alert.description}
                      </p>
                    </div>
                    <Badge
              ...
              <div className="flex items-center justify-between p-2.5 rounded-lg bg-muted/50 border border-border">
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] text-primary border-primary/30 px-1.5"
                >
                  OK
                </Badge>
              </div>
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {y.partner_name || "Unknown partner"}
                        </p>
                      </div>
                    </div>
                    <Badge variant="destructive" className="text-[10px] px-1.5">
                      {typeof y.attendance_pct === "number"
                        ? `${y.attendance_pct.toFixed(1)}%`
                        : `${y.attendance_pct ?? 0}%`}
                    </Badge>
                  </div>
                ))
        ...
        <CardContent>
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() => downloadExportFile("users", "users.xlsx")}
              >
                Export
              </Button>
              <select
          ...
          <div className="flex items-center justify-between mt-3">
            <div className="text-sm text-muted-foreground">
              Showing {(usersData as any)?.rows?.length || users.length} of{" "}
              {(usersData as any)?.total ?? users.length}
            </div>
            <div className="flex items-center gap-2">
              <Button
                variant="ghost"
              ...
              <span className="text-sm">
                ...
                {Math.max(
                  1,
                  Math.ceil(
                    ((usersData as any)?.total || users.length) / usersPageSize,
                  ),
                )}
              </span>
              <Button
                ...
                disabled={
                  ...
                  ((usersData as any).total
                    ? usersPage >=
                      Math.ceil((usersData as any).total / usersPageSize)
            ...
            {actions.map((action) => {
              ...
              return (
                <Button
                  key={action.label}
                  variant={
                    selectedAction === action.label ? "secondary" : "outline"
                  }
                  size="sm"
            ...
            <div className="mt-4 p-4 rounded-xl border border-border bg-background">
              <div className="flex items-center justify-between mb-3 gap-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Interact with the selected dashboard action.
                  </p>
                </div>
                <Button
                  variant="ghost"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                      <Input
                        ...
                        value={userForm.name}
                        className={
                          userFormErrors.name ? "border-destructive" : ""
                        }
                        onChange={(e) => {
                    ...
                    <div>
                      <Input
                        ...
                        className={
                          userFormErrors.email ? "border-destructive" : ""
                        }
                        onChange={(e) => {
                    ...
                    <div>
                      <Input
                        ...
                        className={
                          userFormErrors.password ? "border-destructive" : ""
                        }
                        onChange={(e) => {
                          setUserForm({
                            ...userForm,
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <select
                      ...
                      className="ml-2 rounded-lg border bg-card px-2 py-1 text-sm"
                    >
                      <option value={5}>5</option>
                      <option value={10}>10</option>
                      <option value={25}>25</option>
              ...
              {selectedAction === "Run Sync" && (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    Run a manual metadata sync to refresh attendance and partner
                    records.
                  </p>
                  <Button
              ...
              {selectedAction === "View Audit Log" && (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    Recent admin events and system actions.
                  </p>
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {recentActivity.slice(0, 5).map((item) => (
                      <div
                        key={item.id}
                        className="rounded-xl border border-border p-3 bg-muted/50"
                      >
              ...
              {selectedAction === "Export Data" && (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground">
                    Download data snapshots for reporting or review.
                  </p>
                  <div className="flex flex-wrap gap-2">
                    <Button
                      variant="default"
                    ...
                    <Button
                      ...
                      onClick={() =>
                        downloadExportFile("partners", "partners.xlsx")
                      }
                    >
                      Export Partners
                    </Button>
                    <Button
                      ...
                      onClick={() =>
                        downloadExportFile("sessions", "sessions.xlsx")
                      }
                    >
                      Export Sessions
                    </Button>
